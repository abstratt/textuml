...
public class ReadExtentActionBuilder extends
		ActionBuilder<ReadExtentAction> {
	...
	public void enhanceAction() {
		getProduct().setClassifier(
				this.<Classifier> findNamedElement(classifierName,
						Literals.CLASSIFIER));
		final OutputPin result = getProduct().createResult(null,
				getProduct().getClassifier());
		result.setUpperValue(MDDUtil.createLiteralUnlimitedNatural(
				getProduct().getNearestPackage(),
