...
public class TextUMLFormatterTests extends TestCase {
	...
	public void testEmptyModel() throws CoreException {
		...
		checkFormatting("model bank;<LE><LE>end.", new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testEscaping() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting(
		        "model \\model;<LE><LE>class \\class<LE><LE><TAB>attribute \\attribute : Integer;<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testClassWithAttributes() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting("model bank;<LE><LE>class Foo<LE><LE><TAB>attribute a1 : Integer;<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testIfElse() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting(
		        "model bank;<LE><LE>class Foo<LE><LE><TAB>operation a();<LE><TAB>begin<LE><TAB><TAB>if (condition) then<LE><TAB><TAB><TAB>obj1.method()<LE><TAB><TAB>else<LE><TAB><TAB><TAB>obj2.method();<LE><TAB>end;<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testIfElseif() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting(
		        "model bank;<LE><LE>class Foo<LE><LE><TAB>operation a();<LE><TAB>begin<LE><TAB><TAB>if (condition1) then<LE><TAB><TAB><TAB>obj1.method()<LE><TAB><TAB>elseif (condition2) then<LE><TAB><TAB><TAB>obj2.method();<LE><TAB>end;<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testIfElseifElse() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting(
		        "model bank;<LE><LE>class Foo<LE><LE><TAB>operation a();<LE><TAB>begin<LE><TAB><TAB>if (condition1) then<LE><TAB><TAB><TAB>obj1.method()<LE><TAB><TAB>elseif (condition2) then<LE><TAB><TAB><TAB>obj2.method()<LE><TAB><TAB>else<LE><TAB><TAB><TAB>obj3.method();<LE><TAB>end;<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testIfNoElse() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting(
		        "model bank;<LE><LE>class Foo<LE><LE><TAB>operation a();<LE><TAB>begin<LE><TAB><TAB>if (condition) then<LE><TAB><TAB><TAB>obj1.method();<LE><TAB>end;<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testWordyBlock() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting(
		        "model bank;<LE><LE>class Foo<LE><LE><TAB>operation a();<LE><TAB>begin<LE><TAB><TAB>a := 1;<LE><TAB><TAB>return a + 1;<LE><TAB>end;<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testSimpleBlock() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting(
		        "model bank;<LE><LE>class Foo<LE><LE><TAB>attribute a : Integer := {<LE><TAB><TAB>a := 1;<LE><TAB><TAB>return a + 1;<LE><TAB>};<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testStateMachine() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting(
		        "model bank;<LE><LE>class Foo<LE><LE><TAB>statemachine SM<LE><LE><TAB><TAB>state s1<LE><TAB><TAB><TAB>transition to s2;<LE><TAB><TAB>end;<LE><LE><TAB><TAB>state s2 end;<LE><LE><TAB><TAB>state s3<LE><TAB><TAB><TAB>entry {<LE><TAB><TAB><TAB>};<LE><TAB><TAB><TAB>exit {<LE><TAB><TAB><TAB>};<LE><TAB><TAB>end;<LE><LE><TAB>end;<LE><LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}

	// disabled, this is a bug with identifiers that embed digits that needs
	// fixing
	public void _testClassSpecializes() throws CoreException {
		String toFormat = "model bank; class C1    specializes    C2 end; end.";
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting("model bank;<LE><LE>class C1 specializes C2<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
	...
	public void testOperationParameters() throws CoreException {
		...
		Start parseTree = new TextUMLCompiler().parse(toFormat);
		checkFormatting(
		        "model bank;<LE><LE>class Foo<LE><LE><TAB>operation op1([s] out a1 : Boolean) [v] : Integer;<LE><TAB>begin<LE><TAB>end;<LE>end;<LE><LE>end.",
		        new TextUMLFormatter().format(parseTree, new AnalysisAdapter()));
	}
