...
public class PackageTests extends AbstractRepositoryBuildingTests {
	...
	public void testApplyProfile() throws CoreException {
		...
		Model thisModel = (Model) getRepository().findPackage("someModel",
				IRepository.PACKAGE.getModel());
		Profile someProfile = (Profile) getRepository().findPackage("someProfile",
				IRepository.PACKAGE.getProfile());
	...
	public void testClashBetweenImportedAndLocal() throws CoreException {
		...
		sourcePackage += "end.";
		parseAndCheck(sourcePackage);
		Class localExpressionClass = getRepository().findNamedElement("somePackage::Expression", UMLPackage.Literals.CLASS, null);
	...
	public void testImportModel() throws CoreException {
		...
		Model otherModel = (Model) getRepository().findPackage("otherModel",
				IRepository.PACKAGE.getModel());
	...
	public void testImportPackage() throws CoreException {
		...
		Model thisModel = (Model) getRepository().findPackage("someModel",
				IRepository.PACKAGE.getModel());
		assertNotNull(thisModel);
		Package otherPackage = getRepository().findPackage("otherModel::otherPackage",
	...
	public void testCycleInLookup() throws CoreException {
		...
		Package parentPackage = (Package) getRepository().findPackage("parent",
				IRepository.PACKAGE.getPackage());
		assertNotNull(parentPackage);
		Package childPackage = (Package) getRepository().findPackage("parent::child",
				IRepository.PACKAGE.getPackage());
		assertNotNull(childPackage);
		assertSame(childPackage, getRepository().findNamedElement("child", IRepository.PACKAGE.getPackage(), parentPackage));
		assertSame(parentPackage, getRepository().findNamedElement("parent", IRepository.PACKAGE.getPackage(), childPackage));
	...
	public void testImportPrivatePackage() throws CoreException {
		...
		assertTrue(problems[0].toString(),
				problems[0] instanceof UnresolvedSymbol);
	...
	public void testImportPublicPackage() throws CoreException {
		...
		Class class1 = (Class) getRepository().findNamedElement(
				"modelA::Class1", IRepository.PACKAGE.getClass_(), null);
		Class class2 = (Class) getRepository().findNamedElement(
				"modelC::Class2", IRepository.PACKAGE.getClass_(), null);
	...
	public void testLoadSystemPackage() throws CoreException {
		...
		assertNotNull(getRepository().findPackage("someModel",
				IRepository.PACKAGE.getModel()));
		assertNotNull(getRepository().findNamedElement("UML",
				IRepository.PACKAGE.getPackage(), null));
	}
	...
	public void testLoadAnotherPackage() throws CoreException {
		...
		assertNotNull(getRepository().findPackage("someModel",
				IRepository.PACKAGE.getModel()));
		assertNotNull(getRepository().findNamedElement("library::BaseClass",
	...
	public void testLoadPackageNonExistingLocation() throws CoreException {
		...
		assertEquals(problems[0].toString(), IProblem.Severity.ERROR,
				problems[0].getSeverity());
		assertEquals(CannotLoadFromLocation.class,
				problems[0].getClass());
	}
	...
	public void testModelAndPackageCreation() throws CoreException {
		...
		final Model someModel = (Model) getRepository().findPackage(
				"someModel", IRepository.PACKAGE.getPackage());
		assertNotNull(someModel);
		final Package somePackage = getRepository().findPackage(
				"someModel::somePackage", IRepository.PACKAGE.getPackage());
	...
	public void testNestedPackage() throws CoreException {
		...
		assertNotNull(getRepository().findPackage("parent",
				IRepository.PACKAGE.getPackage()));
		assertNotNull(getRepository().findPackage("parent::child1",
				IRepository.PACKAGE.getPackage()));
		assertNotNull(getRepository().findNamedElement("parent::child1::Class1",
				IRepository.PACKAGE.getClass_(), null));
	}
	...
	public void testNestedPackageTwoSeparateCU() throws CoreException {
		...
		assertNotNull(getRepository().findPackage("parent::child1",
				IRepository.PACKAGE.getPackage()));
		assertNotNull(getRepository().findPackage("parent::child2",
				IRepository.PACKAGE.getPackage()));
		assertNotNull(getRepository().findNamedElement("parent::child1::Class1",
				IRepository.PACKAGE.getClass_(), null));
		assertNotNull(getRepository().findNamedElement("parent::child2::Class2",
				IRepository.PACKAGE.getClass_(), null));
	}
	...
	public void test3LevelNestedPackage() throws CoreException {
		...
		// normal order
		parseAndCheck( firstPackage, secondPackage, thirdPackage);
		assertNotNull(getRepository().findPackage("first",
				IRepository.PACKAGE.getPackage()));
		assertNotNull(getRepository().findPackage("first::second",
				IRepository.PACKAGE.getPackage()));
		assertNotNull(getRepository().findPackage("first::second::third",
				IRepository.PACKAGE.getPackage()));
	}
	...
	public void test3LevelNestedPackageXRef() throws CoreException {
		...
		assertNotNull(getRepository().findPackage("first::second::third",
				IRepository.PACKAGE.getPackage()));
		assertNotNull(getRepository().findNamedElement("first::First",
				IRepository.PACKAGE.getClass_(), null));
		assertNotNull(getRepository().findNamedElement("first::second::Second",
				IRepository.PACKAGE.getClass_(), null));
		assertNotNull(getRepository().findNamedElement("first::second::third::Third",
				IRepository.PACKAGE.getClass_(), null));
	}
	...
	public void testPackageCreation() throws CoreException {
		...
		source += "end.";
		parseAndCheck(source);
		final Package somePackage = getRepository().findPackage("somePackage",
				IRepository.PACKAGE.getPackage());
		assertNotNull(somePackage);
	}
	...
	public void testNestedPackageSingleCU() throws CoreException {
		...
		parseAndCheck(source);
		final Model someModel = (Model) getRepository().findPackage(
				"someModel", IRepository.PACKAGE.getModel());
		assertNotNull(someModel);
		final Package somePackage = getRepository().findPackage(
				"someModel::somePackage", IRepository.PACKAGE.getPackage());
		assertNotNull(somePackage);
		...
		final Class fooClass = getRepository().findNamedElement(
				"someModel::Foo", IRepository.PACKAGE.getClass_(), null);
		assertNotNull(fooClass);
		assertSame(someModel, fooClass.getOwner());
		final Class barClass = getRepository().findNamedElement(
				"someModel::somePackage::Bar", IRepository.PACKAGE.getClass_(), null);
		assertNotNull(barClass);
		assertSame(somePackage, barClass.getOwner());
		final Class fredClass = getRepository().findNamedElement(
				"someModel::Fred", IRepository.PACKAGE.getClass_(), null);
	...
	public void testProfileCreation() throws CoreException {
		...
		source += "end.";
		parseAndCheck(source);
		final Profile profile = (Profile) getRepository().findPackage(
				"someProfile", IRepository.PACKAGE.getProfile());
	...
	public void testInvalidProfileCreation() throws CoreException {
		...
		assertEquals(problems[0].toString(), IProblem.Severity.ERROR,
				problems[0].getSeverity());
		assertEquals(InvalidPackageNesting.class.getName(),
				problems[0].getClass().getName());
	}
	...
	public void testInvalidModelCreation() throws CoreException {
		...
		assertEquals(problems[0].toString(), IProblem.Severity.ERROR,
				problems[0].getSeverity());
		assertEquals(InvalidPackageNesting.class,
				problems[0].getClass());
	}
}
