...
public class ActivityUtils {
	 ...
	 *  
	 * @param startingPoint starting point node, will fall back to parents if required
	 * @param exceptionType the type of exception to be handled
	...
	public static Variable findVariable(StructuredActivityNode node, String variableName) {
		...
		if (MDDExtensionUtils.isClosure(node.getOwner()))
			// this is the outermost block of a closure, fall back to the closure context
   ...
   public static Activity getActionActivity(Action action) {
	...
	public static ValueSpecification buildBehaviorReference(Package parent, Activity activity, Type type) {
		OpaqueExpression expression = (OpaqueExpression) parent.createPackagedElement(null, IRepository.PACKAGE.getOpaqueExpression());
	...
	/** A way of getting an action's input pins that will work for StructuredActivityNodes. */
	...
	/** A way of getting an action's output pins that will work for StructuredActivityNodes. */
	...
	public static void createParameterVariables(Activity activity) {
		...
		for (Parameter parameter : activity.getOwnedParameters()) {
			...
			String varName =
							parameter.getDirection() == ParameterDirectionKind.RETURN_LITERAL ? "" : parameter
											.getName();
	...
	/**
	 * A cast is a {@link StructuredActivityNode} with only two nodes: an input and an output.
	 * TODO an ObjectFlow would be more appropriate
	 */
    ...
    public static List<Action> findStatements(StructuredActivityNode target) {
        ...
        for (ActivityNode node : target.getNodes()) 
            if (node instanceof Action && ActivityUtils.isTerminal((Action) node))
    ...
    public static void makeFinal(StructuredActivityNode block, Action lastAction) {
        ActivityFinalNode finalNode = (ActivityFinalNode) block.createNode(null, UMLPackage.Literals.ACTIVITY_FINAL_NODE);
	...
	public static BehavioredClassifier getContext(Behavior behavior) {
		...
		if (standardContext == null && (behavior.getOwner() instanceof BehavioredClassifier))
			// bug in UML2? During a tycho build, a behavior owned by a behaviored classifier would be null sometimes
	...
	public static Action getFinalAction(StructuredActivityNode startingPoint) {
		ActivityFinalNode finalNode = (ActivityFinalNode) ActivityUtils.findNode(startingPoint, new MDDUtil.EClassMatcher(Literals.ACTIVITY_FINAL_NODE));
		if (finalNode == null)
			return null;
