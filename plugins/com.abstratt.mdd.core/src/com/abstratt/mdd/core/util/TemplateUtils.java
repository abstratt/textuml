...
public class TemplateUtils {
	...
	/**
	 * Applies substitutions to a parameterable element in the context of a bound element.
	 * 
	 * @param <PE>
	 * @param <TE>
	 * @param templateable
...
//						findTemplateParameterSubstitutions(templateable, parameterable.getTemplateParameter()
//										.getSignature());
...
//			return null;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <TE extends TemplateableElement, PE extends ParameterableElement> TE createBinding(org.eclipse.uml2.uml.Package namespace, TE template,
					List<PE> actualParams) {
		...
		TE result = (TE) namespace.createPackagedElement(null, template.eClass());
		NamedElement namedResult = (NamedElement) result;
		namedResult.setName(generateTemplateInstanceName(template, actualParams));
	...
	private static <TE, PE> String generateTemplateInstanceName(TE template,
			List<PE> actualParams) {
	...
	static boolean isCompatible(TemplateableElement source, TemplateableElement destination) {
		...
		for (TemplateBinding sourceBinding : sourceBindings) {
			...
			final EList<TemplateParameterSubstitution> sourceParameterSubstitutions =
							sourceBinding.getParameterSubstitutions();
			final EList<TemplateParameterSubstitution> destinationParameterSubstitutions =
							destinationBinding.getParameterSubstitutions();
			...
			for (int i = 0; i < sourceParameterSubstitutions.size(); i++) {
				...
				if (!UML2Compatibility.getActualParameter(sourceSubstitution).equals(UML2Compatibility.getActualParameter(destinationSubstitution)))
					return false;
			}
		}
