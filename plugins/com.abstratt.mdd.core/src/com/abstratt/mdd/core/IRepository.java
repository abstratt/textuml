...
public interface IRepository extends IBasicRepository {
	...
	/**
	 * Returns the first element in the repository to satisfy the given condition.
	 * 
	 * @param condition condition an element must satisfy
	 * @return the first element to satisfy the condition, or <code>null</code> if none
	 */
	public Element findFirst(EObjectCondition condition);
	
	/**
	 * Returns the all elements in the repository that satisfy the given condition.
	 * 
	 * @param condition condition an element must satisfy
	 * @param internalOnly whether only elements of the repository should be considered 
	...
	/**
	 * Creates a new package from the given qualified name. If a package 
	 * with the given qualified name already exists, just returns it.
	 * 
	 * @param qualifiedName
	...
	/**
	 * Creates a new top-level package from the given qualified name. If a package 
	 * with the given qualified name already exists, just returns it.
	 * 
	 * @param name
	 * @param packageClass the package type, or <code>null<code>
	 * @param resourceURI the resource location, or <code>null</code> 
	 * if the default location should be used
	 * @return	
	 */
	public Package createTopLevelPackage(String qualifiedName, EClass packageClass, URI resourceURI);

	/**
	 * Disposes this repository instance. Afterwards, any attempts to use it will fail.
	 */
	public void dispose();
	
	/**
	 * Whether this repository is open. A repository is open until it is disposed.
	 * 
	...
	/**
	 * Returns the package with the given qualified name and package type. This is a convenience method, 
	 * equivalent to:
	 * <code>
	 * {@link #findNamedElement(qualifiedName, packageClass, scope)}
	 * </code>
	 * 
	 * @param qualifiedName the qualified name for the package
	 * @param packageClass the package type or <code>null</code> if any
	...
	/**
	 * Returns the operation in the given package that is an entry point, 
	 * or <code>null</code> if none is found. If multiple entry point 
	 * operations exist, one of them will be returned.
	 * 
	 * @param package_ the package where to look for entry point operations
	 * @return an operation, or <code>null</code>
	 */
	public Operation getEntryPointOperation(Package package_);

	/**
	 * Returns all top-level packages of the given type. If the package class is not 
	 * provided, returns all top-level packages.
	 * . 
	 * @param packageClass
	 ...
	 * 
	 * @param name the qualified name of a package
	 * @return the package found, or <code>null</code>	
	 */
	public Package loadPackage(String name);

	/**
	 * Loads an existing package from the given URI. If a package 
	 * with the given URI cannot be found, returns <code>null</code>.
	 * 
	 * @param packageURI the URI where to find a package
	 * @return the package found, or <code>null</code>	
	 */
	public Package loadPackage(URI packageURI);

	/**
	 * Removes a package with the given qualified name. Does nothing
	 * if a package with the given name does not exist.
	 * 
	 * @param name the qualified name for a package
	 */
	public void removePackage(String name);

	/**
	 * Runs the given builder runnable having this repository as the current in-progress repository.
	 * 
	 * @param runnable a runnable that builds this repository up
