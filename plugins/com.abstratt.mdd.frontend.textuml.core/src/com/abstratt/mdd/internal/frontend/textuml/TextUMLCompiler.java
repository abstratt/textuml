...
public class TextUMLCompiler implements ICompiler, ISourceAnalyzer {
        ...
        try {
            ...
            parsed.getPStart().apply(new DepthFirstAdapter() {
                ...
                public void caseAStart(AStart node) {
                    ((APackageHeading) node.getPackageHeading()).getQualifiedIdentifier().apply(
     ...
     * 
     * @see
     * com.abstratt.mdd.core.frontend.spi.ICompiler#format(java.lang.String)
     */
        ...
        try {
            ...
            if (problems != null)
                problemBuilder.addProblem(
                        new SyntaxProblem("Found: '" + e.getToken().getText() + "'. " + e.getMessage()), e.getToken());
            ...
            IStatus status = new Status(IStatus.ERROR, TextUMLConstants.PLUGIN_ID, 0, "Error reading source unit: "
                    + source.toString(), e);
     ...
     *
     * @param toParse
     *            source of compilation unit to be parsed
    ...
    /**
     * Given a position in a compilation unit, finds the contextual package
     * name.
     * 
     * @param toParse
     *            source of compilation unit
     * @param line
        ...
        for (Node current = token; current != null; current = current.parent()) {
            current.apply(new AnalysisAdapter() {
                ...
                public void caseAAssociationDef(AAssociationDef node) {
                    final String associationName = ((AAssociationHeader) node.getAssociationHeader()).getIdentifier()
                            .getText();
    ...
    /**
     * Returns the token found at the given position in the given compilation
     * unit.
     * 
     * @param toParse
     *            source of compilation unit
     ...
     *            whether whitespaces should be ignored
     * @param ignoreSemicolon
     *            whether semicolons should be ignored
     * @return the token found
     */
    public Token findTokenAt(String toParse, final int line, final int col, final boolean ignoreWhitespace,
            final boolean ignoreSemicolon) {
    ...
    public List<SourceElement> analyze(String source) {
        ...
        parsed.apply(new DepthFirstAdapter() {
            ...
            public void caseAClassDef(AClassDef node) {
                ...
                SourceElement element = new SourceElement(classTypeName + " " + className, sourceMiner
                        .getLineNumber(node.getClassHeader()), kind);
            ...
            @Override
            public void caseAAssociationDef(AAssociationDef node) {
                PAssociationKind assocKind = sourceMiner.findChild(node.getAssociationHeader(), PAssociationKind.class,
                        true);
                ...
                SourceElement element = new SourceElement(assocKindTypeName + " " + StringUtils.trimToEmpty(assocName),
                        sourceMiner.getLineNumber(node.getAssociationHeader()), ElementKind.Associations);
                elements.add(element);
                parent.add(element);
            ...
            public void caseAStateMachineDecl(AStateMachineDecl node) {
                ...
                SourceElement element = new SourceElement("statemachine" + " " + smName, sourceMiner.getLineNumber(node
                        .getIdentifier()), ElementKind.StateMachines);
                parent.get(parent.size() - 1).getChildren().add(element);
                parent.add(element);
            ...
            public void caseAStateDecl(AStateDecl node) {
                ...
                String label = StringUtils.isBlank(stateName) ? modifier : (StringUtils.isBlank(modifier) ? stateName
                        : (stateName + " (" + modifier + ")"));
                SourceElement element = new SourceElement(label, sourceMiner.getLineNumber(node.getState()),
                        ElementKind.States);
                parent.get(parent.size() - 1).getChildren().add(element);
            }
            ...
            public void caseAReferenceDecl(AReferenceDecl node) {
                ...
                ElementKind elementKind = refType instanceof ACompositionReferenceType ? ElementKind.Compositions
                        : (refType instanceof AAggregationReferenceType ? ElementKind.Aggregations
                                : ElementKind.References);
            ...
            public void caseAAttributeDecl(AAttributeDecl node) {
                addAttribute(isPrivateFeature(node), node.getIdentifier(), node.getTypeIdentifier(),
                        ElementKind.Attributes);
            }
            ...
            private void addAttribute(boolean isPrivate, Node identifierNode, Node typeNode, ElementKind elementKind) {
                ...
                SourceElement element = new SourceElement((isPrivate ? "-" : "") + attributeName + " : "
                        + attributeType, sourceMiner.getLineNumber(identifierNode), elementKind);
                parent.get(parent.size() - 1).getChildren().add(element);
            }
            ...
            public void caseAOperationDecl(AOperationDecl node) {
                ...
                ASimpleOptionalReturnType returnType = sourceMiner.findChild(header.getSignature(),
                        ASimpleOptionalReturnType.class, true);
                ...
                for (ASimpleParamDecl paramDecl : sourceMiner.findChildren(signature.getParamDeclList(),
                        ASimpleParamDecl.class))
                    parameterTypes.add(sourceMiner.getText(paramDecl.getTypeIdentifier()));
                String allParameters = StringUtils.join(parameterTypes, ", ");
                String representation = (isPrivate ? "-" : "") + operationName + (isQuery ? "?" : "") + "("
                        + allParameters + ")" + operationType;
                SourceElement element = new SourceElement(representation, sourceMiner.getLineNumber(header
                        .getOperationKeyword()), ElementKind.Operations);
                parent.get(parent.size() - 1).getChildren().add(element);
            }

            protected boolean isPrivateFeature(Node node) {
                return sourceMiner.findChild(sourceMiner.findParent(node, AFeatureDecl.class).getModifiers(),
                        TPrivate.class, true) != null;
            }

            @Override
            public void caseAReceptionDecl(AReceptionDecl node) {
                ATypeIdentifier signalType = sourceMiner.findChild(node.getSimpleParamDecl(), ATypeIdentifier.class,
                        true);
                String operationName = node.getReceptionName().getText();
                String representation = operationName + "(" + signalType + ")";
                SourceElement element = new SourceElement(representation, node.getReception().getLine(),
                        ElementKind.Receptions);
                parent.get(parent.size() - 1).getChildren().add(element);
            }
        });
        ...
        if (elements.size() > 10) {
            ...
            for (SourceElement sourceElement : elements) {
                ...
                if (group == null)
                    groups.put(sourceElement.getKind(), group = new SourceElement(sourceElement.getKind().name(),
                            sourceElement.getLine(), ElementKind.Groups));
