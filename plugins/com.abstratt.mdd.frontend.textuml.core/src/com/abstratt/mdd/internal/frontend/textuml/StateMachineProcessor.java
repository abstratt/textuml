...
import com.abstratt.mdd.internal.frontend.textuml.node.ACallTransitionTrigger;
import com.abstratt.mdd.internal.frontend.textuml.node.AEntryStateBehaviorModifier;
import com.abstratt.mdd.internal.frontend.textuml.node.AExitStateBehaviorModifier;
import com.abstratt.mdd.internal.frontend.textuml.node.ASignalTransitionTrigger;
import com.abstratt.mdd.internal.frontend.textuml.node.AStateBehavior;
import com.abstratt.mdd.internal.frontend.textuml.node.AStateDecl;
import com.abstratt.mdd.internal.frontend.textuml.node.AStateMachineDecl;
import com.abstratt.mdd.internal.frontend.textuml.node.ATransitionDecl;
import com.abstratt.mdd.internal.frontend.textuml.node.Node;
import com.abstratt.mdd.internal.frontend.textuml.node.PStateBehavior;
import com.abstratt.mdd.internal.frontend.textuml.node.PStateBehaviorModifier;
import com.abstratt.mdd.internal.frontend.textuml.node.PStateDecl;
import com.abstratt.mdd.internal.frontend.textuml.node.PTransitionDecl;
import com.abstratt.mdd.internal.frontend.textuml.node.PTransitionTrigger;
...
public class StateMachineProcessor extends AbstractProcessor<AStateMachineDecl, BehavioredClassifier>{
	...
	private class StateProcessor implements NodeProcessor<AStateDecl> {
		...
		public void process(AStateDecl node) {
			...
			if (stateName != null && region.getSubvertex(stateName) != null) {
				problemBuilder.addProblem(new UnclassifiedProblem("A state with this name already exists"), node.getState());
				return;
			}
			...
			if (isInitial && isTerminate) {
				problemBuilder.addProblem(new UnclassifiedProblem("Can't be both initial and terminate"), node.getState());
				return;
			}
			if (isInitial || isTerminate) {
				Pseudostate pseudoState = (Pseudostate) region.createSubvertex(stateName, UMLPackage.Literals.PSEUDOSTATE);
			...
			if (!node.getStateBehavior().isEmpty()) {
				...
				for (PStateBehavior pStateBehavior : node.getStateBehavior()) {
					...
					problemBuilder.ensure(aStateBehavior.getStateBehaviorDefinition() instanceof ABehaviorStateBehaviorDefinition, "Only inline behavior definition supported at this time", pStateBehavior);
					...
					if (modifier instanceof AEntryStateBehaviorModifier) {
						problemBuilder.ensure(asState.getEntry() == null, "Only one entry activity allowed", pStateBehavior);
						asState.setEntry(activity);
					} else if (modifier instanceof AExitStateBehaviorModifier) {
						problemBuilder.ensure(asState.getExit() == null, "Only one exit activity allowed", pStateBehavior);
						asState.setExit(activity);
					} else {
						problemBuilder.ensure(asState.getDoActivity() == null, "Only one do activity allowed", pStateBehavior);
	...
	public class TransitionProcessor implements NodeProcessor<ATransitionDecl> {
		...
		public void process(final ATransitionDecl node) {
			...
			referenceTracker.add(new DeferredReference<Vertex>(destinationName, UMLPackage.Literals.VERTEX, namespaceTracker.currentNamespace(null)) {
				@Override
			...
			if (node.getTransitionTriggers() != null)
				node.getTransitionTriggers().apply(new DepthFirstAdapter() {
					...
					public void caseACallTransitionTrigger(ACallTransitionTrigger node) {
						new TriggerProcessor<PTransitionTrigger>(transition, UMLPackage.Literals.OPERATION, UMLPackage.Literals.CALL_EVENT).process(node);
					}
					@Override
					public void caseASignalTransitionTrigger(
							ASignalTransitionTrigger node) {
						new TriggerProcessor<PTransitionTrigger>(transition, UMLPackage.Literals.SIGNAL, UMLPackage.Literals.SIGNAL_EVENT).process(node);
					}
					@Override
					public void caseAAnyTransitionTrigger(AAnyTransitionTrigger node) {
						new TriggerProcessor<PTransitionTrigger>(transition, null, UMLPackage.Literals.ANY_RECEIVE_EVENT).process(node);
					}
				});
			if (node.getTransitionGuard() != null)
				behaviorGenerator.createConstraintBehaviorLater(source.containingStateMachine(), transition.createGuard(null), node.getTransitionGuard(), Collections.<Parameter>emptyList());
			if (node.getTransitionEffect() != null)
				behaviorGenerator.createBodyLater(node.getTransitionEffect(), (Activity) transition.createEffect(null, UMLPackage.Literals.ACTIVITY));
		}
	}
	private BehaviorGenerator behaviorGenerator;

	public StateMachineProcessor(SourceCompilationContext<Node> compilationContext, BehavioredClassifier contextClassifier) {
	...
	public void process(AStateMachineDecl node) {
		...
		try {
			...
			if (region.getSubvertices().isEmpty()) {
				problemBuilder.addProblem(new UnclassifiedProblem("Can't have a state machine without states"), node.getStatemachine());
				return;
		    }
			int initialCount = 0;
